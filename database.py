...
class Database:
    ...
    # -------------
    def get_from_client_local_settings(self, client_id):
        query = 'select * from client_local_settings where client_id = %s'
        args = (client_id,)
        ret = False
        try:
    ...
    def set_to_client_local_setting(self, client_id, data):
        query = 'update client_local_settings set first=%s, end=%s, offset=%s, last=%s, running_mod=%s, ' \
                'max_thread=%s, execute_status=%s where client_id = %s'
        args = (data['first'], data['end'], data['offset'], data['last'], data['running_mod'], data['max_thread'],
                data['execute_status'], client_id)

        return self.command_query(query, args, True)

    def get_main_stop_flag(self, client_id):
        res = self.get_from_client_local_settings(client_id)
        if res is not False:
    ...
    def get_database_record0(self):
        query = 'select index_data.count(date_m), index_info.count(index_id), open_days.count(date_m), share_daily_data.count(date_m), share_adjusted_data.count(date_m), share_info.count(en_symbol_12_digit_code), share_second_data.count(date_time), share_status.count(date_m), share_sub_trad_data.count(date_m), shareholders_data.count(date_m) ' \
                'from index_data, index_info, open_days, share_daily_data, share_adjusted_data, share_info, share_second_data, share_status, share_sub_trad_data, shareholders_data'
        args = ()

        res = self.select_query(query, args, 1)
        if res is not False:
            sum_record = int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0]) + int(res[0][0])
